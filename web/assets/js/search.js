...
$(document).ready(function () {
        ...
        success: function (products) {
            products.forEach(function (product) {
                if (queryParams.search) {
                    ...
                    if (
                        product.nome.toLowerCase().indexOf(decodedSearch.toLowerCase()) === -1 &&
                        product.descricao.toLowerCase().indexOf(decodedSearch.toLowerCase()) === -1
                    ) {
                        return;
                    }
                }
	...
	$.ajax({
        url: 'http://localhost/trabalho/server/produtos/categorias.php',
data: {
	categoria: queryParams.category.replace(/%27/g, "'")
},
        method: 'GET',
        success: function (products) {
            products.forEach(function (product) {
